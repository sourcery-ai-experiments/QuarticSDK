...
def step_impl(context):
    ...
    with mock.patch('requests.get') as requests_get:
        requests_get.return_value = APIHelperCallAPI(TestHelpers.ASSET_LIST_GET)
    ...
    with mock.patch('requests.get') as requests_get:
        requests_get.return_value = APIHelperCallAPI(TestHelpers.TAG_LIST_GET)

        world.first_asset_tags = world.first_asset.get_tags()

    with mock.patch('requests.get') as requests_get:
        requests_get.return_value = APIHelperCallAPI(TestHelpers.BATCH_LIST_GET)

        world.first_asset_batches = world.first_asset.batches()

    with mock.patch('requests.post') as requests_post:
        requests_post.return_value = APIHelperCallAPI(TestHelpers.ASSET_DATA_POST)

        world.first_asset_data = world.first_asset.data(start_time=1, stop_time=2)

    world.first_tag = world.first_asset_tags.first()

    with mock.patch('requests.post') as requests_post:
        requests_post.return_value = APIHelperCallAPI(TestHelpers.TAG_DATA_POST)

        world.first_tag_data = world.first_tag.data(start_time=1,stop_time=2)

    with mock.patch('requests.get') as requests_get:
        requests_get.return_value = APIHelperCallAPI(TestHelpers.CONTEXT_FRAME_LIST_GET)
    ...
    with mock.patch('requests.get') as requests_get:
        requests_get.return_value = APIHelperCallAPI(TestHelpers.CONTEXT_FRAME_OCCURRENCE_GET)
...
def step_impl(context):
    ...
    assert isinstance(world.first_asset_data, TagDataIterator)
